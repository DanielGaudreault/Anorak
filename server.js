...
const app = express();
const PORT = 3000;
const GOOGLE_API_KEY = 'AIzaSyCksB_LXCIFFR9o2OrCK7i-nsEqX9tvplE';
...
app.post('/talk-to-anorak', async (req, res) => {
    ...
    if (!OPENAI_API_KEY || OPENAI_API_KEY === 'YOUR_OPENAI_API_KEY') {
        console.warn('OpenAI API key missing, skipping OpenAI');
    } else {
        try {
    ...
    try {
        ...
        return res.status(500).json({ error: 'The OASIS archives are down! Falling back to local wisdom.' });
    }
});
